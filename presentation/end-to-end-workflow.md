...
The executable spine is **`scripts/replicate_paper.sh`**. It binds ten stages in order; **`scripts/run_budget_pipeline.sh`** is a convenience layer that sets typical **budget** hyperparameters without editing the orchestrator. Numbers in **§3** mirror the current **`run_budget_pipeline.sh`** defaults.
...
- Honors **environment variables** for decoding limits, caps on dataset rows, merge sizes, optional step-8 only flags (see **`replicate_paper.sh`** header and §3).
...
## 3. Reference configuration (budget run)

When you invoke **`scripts/run_budget_pipeline.sh`**, it forwards the following baseline into **`replicate_paper.sh`** (overridable via `export` before running):

| Knob | Value | Role |
|------|-------|------|
| `OUT_DIR` | `runs/my-new-budget-run` (default) | Root for all checkpoints and logs |
| `BASE_MODEL` | `Qwen/Qwen2.5-1.5B-Instruct` | Initialization for both SFT stages and RLOO |
| `SCALE` | `paper` | Data fetch & artifact prep preset (full upstream scale, richer trace augmentation) |
| `RLOO_STEPS` | `100` | Policy-gradient optimization steps |
| `MAX_CLEAN_TRIES` | `3` | Max `<clean>`-driven retries per example at final eval |
| `RECOVERY_STYLE` | `grounded` | How failed rollouts become supervised recovery traces |
| `LTR_SFT_MAX_TRAIN_EXAMPLES` | `10000` | Cap on **first** SFT epoch rows |
| `LTR_EVAL_RAW_MAX_EXAMPLES` | `750` | Step-4 raw decode sample count for mining pool |
| `LTR_MERGE_MAX_BASE_EXAMPLES` | `10000` | Max **base** SFT rows in combined corpus (after conversion) |
| `LTR_MERGE_MAX_MINED_EXAMPLES` | `750` | Max **mined** rows in combined corpus |
| `LTR_RLOO_MAX_TRAIN_EXAMPLES` | `10000` | Countdown train prompts loaded into RLOO |
| `LTR_RLOO_MAX_VALIDATION_EXAMPLES` | `25000` | Countdown validation prompts for RLOO metrics |
| `LTR_MAX_NEW_TOKENS` | `768` | Generation horizon for raw eval, RLOO rollout, final eval |

**Unset by default:** `LTR_EVAL_FINAL_MAX_EXAMPLES` — final eval runs the **full** hard split unless you cap it for smoke tests. **`LTR_VERIFIER_FEEDBACK`** stays `0` (no oracle hints appended on retries).

**Fixed inside the orchestrator (not exposed as env yet):** SFT **`--epochs 1`**, **`sft_runtime`** default **`max_length` 1024** for supervised packing, second SFT consumes the **full** merged file (no arbitrary head truncations), RLOO **`responses-per-prompt 4`**, **`temperature 1.0`**.
...
**Algorithm.** **Greedy (or temperature-0) single-pass** generation: one completion per prompt, scored by the Countdown verifier (see §8). **Why:** We want a clean event log of “what the current policy does wrong” before we teach recovery.
